...
ecosystem. Use [uv](../reproducibility/package-management.qmd) to install pytest into
...
-   `"class"`: Fixture is run once for each test class and used within any function that
    calls it.
-   `"module"`: Fixture is run once for each module and used within any function that
    calls it.
